...
function container()
{
	...
	for(var a=0;a<wearer.length;a++)
	{
		...
		for each(artLoc in curArtLayers)
		{
			...
			if(!success)
			{
				errorList.push("Failed while trying to add " + artLoc);
				sendErrors(errorList);
				valid = false;
				return;
			}
			else
			{
				try
				{
					//try to move artwork to coords stored in library[code].placement
				...
				{
					errorList.push("Failed while moving the pieces for " + wearer[a].name);
					sendErrors(errorList);
					valid = false;
					return;
				}
